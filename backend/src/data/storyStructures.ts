...
export function recommendStructure(panelCount: number): StoryStructureTemplate {
  if (panelCount === 4) {
    return STORY_STRUCTURES[0] as StoryStructureTemplate; // 起承转合
  } else if (panelCount >= 6 && panelCount <= 8) {
    return STORY_STRUCTURES[1] as StoryStructureTemplate; // 三幕剧
  } else if (panelCount >= 10) {
    return STORY_STRUCTURES[2] as StoryStructureTemplate; // 英雄之旅
  } else {
    // 默认返回起承转合
